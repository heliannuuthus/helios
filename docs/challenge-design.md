...
前端                                    后端
 ...
 │  (用户完成 Turnstile)                  │
 │                                       │
 │  POST /auth/challenge/xxx/captcha     │
 ...
 │  (用户输入验证码)                      │
 │                                       │
 │  POST /auth/challenge/xxx/email_otp   │
 │  { proof: "382910" }                  │
 │ ────────────────────────────────────> │
...
前端                                    后端
 ...
 │ <──────────────────────────────────── │
 │                                       │
 │  POST /auth/challenge/zzz/captcha     │
 │  { proof: "turnstile_token" }         │
 │ ────────────────────────────────────> │
 │                                       │
 │  { verified: true }                   │
...
Verify 接口通过 URL 路径中的 connection 参数显式分发：

```
POST /auth/challenge/:challenge_id/:connection

switch connection {
case "captcha":
    → 如果 challenge.Type == captcha → 验证 captcha challenge
    → 如果 pending_captcha == true  → 验证前置 captcha，通过后触发副作用
...
`ConnectionConfig.Delegate` 表示"可以替代该 IDP 主认证的独立验证方式"：
...
| 字段 | 关系 | 语义 |
|------|------|------|
| `strategy` | OR | 同一 connection 下的主认证方式，选一种 |
| `delegate` | OR | 可替代主认证的独立验证方式，选一种 |
...
- **Strategy**（如 password / passkey）：由 IDP 自身处理验证，proof 直接提交给 IDP
- **Delegate**（如 email_otp / totp）：由 Challenge 流程独立完成验证，proof 是 ChallengeToken

两者是**同级替代关系**——用户可以选择密码登录，也可以选择邮件验证码登录。
