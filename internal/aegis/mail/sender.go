...
import (
	...
	"github.com/heliannuuthus/helios/internal/aegis/mail/templates"
	"github.com/heliannuuthus/helios/pkg/logger"
	"github.com/heliannuuthus/helios/pkg/mail"
)

// Sender 邮件发送器（适配 aegis 模块）
type Sender struct {
	client         *mail.Client
...
func NewSender(cfg *SenderConfig) *Sender {
	...
	if cfg.UseSSL {
		...
		opts = append(opts, mail.WithSTARTTLS())
	}

	client := mail.NewClient(cfg.Host, cfg.Username, cfg.Password, opts...)
...
// ==================== 实现 authenticate.EmailSender 接口 ====================

// Send 发送邮件（实现 authenticate.EmailSender 接口）
...
// GetClient 获取底层邮件客户端
func (s *Sender) GetClient() *mail.Client {
	return s.client
}
