// Package oper provides the B-end operator password login IDP.
package oper
...
// Provider B 端运营人员账号密码 Provider
type Provider struct {
	userSvc *hermes.UserService
}

// NewProvider 创建 B 端运营人员 Provider
...
func (p *Provider) Login(ctx context.Context, proof string, params ...any) (*models.TUserInfo, error) {
	...
	logger.Infof("[oper] 登录请求 - Identifier: %s", maskIdentifier(identifier))
...
func (p *Provider) loginByPassword(ctx context.Context, identifier, password string) (*models.TUserInfo, error) {
	...
	if err != nil {
		logger.Warnf("[oper] 用户不存在 - Identifier: %s, Error: %v", maskIdentifier(identifier), err)
		return nil, errors.New("invalid credentials")
	}

	if cred.Status != 0 {
		logger.Warnf("[oper] 用户已禁用 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("user is disabled")
	}

	if cred.PasswordHash == "" {
		logger.Warnf("[oper] 用户未设置密码 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("password not set, please use other login methods")
	}

	if err := bcrypt.CompareHashAndPassword([]byte(cred.PasswordHash), []byte(password)); err != nil {
		logger.Warnf("[oper] 密码错误 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("invalid credentials")
	}

	logger.Infof("[oper] 登录成功 - Identifier: %s, OpenID: %s", maskIdentifier(identifier), cred.OpenID)
	...
	return &models.TUserInfo{
		...
		RawData:  fmt.Sprintf(`{"identifier":"%s","type":"oper"}`, identifier),
	}, nil
}

// getCredential 获取 B 端运营人员凭证
func (p *Provider) getCredential(ctx context.Context, identifier string) (*credential, error) {
	cred, err := p.userSvc.GetOperByIdentifier(ctx, identifier)
	if err != nil {
		return nil, err
	}
	return &credential{
...
func (*Provider) FetchAdditionalInfo(_ context.Context, infoType string, _ ...any) (*idp.AdditionalInfo, error) {
	return nil, fmt.Errorf("oper provider does not support fetching %s", infoType)
}
