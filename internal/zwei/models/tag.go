...
type RecipeTag struct {
	ID        uint      `gorm:"primaryKey;autoIncrement;column:_id" json:"-"`
	RecipeID  string    `gorm:"not null;index;column:recipe_id;size:32" json:"-"` // 关联 recipe.recipe_id
	TagValue  string    `gorm:"not null;index;column:tag_value;size:50" json:"-"` // 关联 tag.value
	TagType   TagType   `gorm:"not null;index;column:tag_type;size:20" json:"-"`  // 关联 tag.type（冗余字段，优化查询）
	CreatedAt time.Time `gorm:"not null;column:created_at" json:"created_at"`
}
