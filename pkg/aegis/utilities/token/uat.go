...
type UserAccessToken struct {
	Claims
	scope    string
	actor    string // 代理应用 ID（client_credentials delegation 时设置）
	identity *userInfo
}
...
func (u *UAT) Actor(actor string) *UAT {
	u.actor = actor
	return u
}
...
func ParseUserAccessToken(pasetoToken *paseto.Token) (*UserAccessToken, error) {
	...
	var actor string
	if err := pasetoToken.Get(ClaimAct, &actor); err != nil {
		actor = ""
	}
...
func (u *UserAccessToken) Build() (*paseto.Token, error) {
	...
	if u.actor != "" {
		if err := t.Set(ClaimAct, u.actor); err != nil {
			return nil, fmt.Errorf("set act: %w", err)
		}
	}
	return &t, nil
}
...
// Actor 返回代理者应用 ID（client_credentials delegation 时为代理应用的 app_id）。
func (u *UserAccessToken) Actor() string {
	return u.actor
}

// IsDelegated 返回该 UAT 是否为代理签发。
func (u *UserAccessToken) IsDelegated() bool {
	return u.actor != ""
}
