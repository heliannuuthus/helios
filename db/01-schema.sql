...
CREATE TABLE IF NOT EXISTS ingredients (
    ...
    category        VARCHAR(32),                -- 关联 ingredient_categories.key
    quantity        REAL,                       -- 数量
    unit            VARCHAR(16),                -- 单位
...
CREATE TABLE IF NOT EXISTS view_history (
    _id         INTEGER PRIMARY KEY AUTOINCREMENT,
    openid      VARCHAR(64) NOT NULL,           -- 用户 openid
    recipe_id   VARCHAR(64) NOT NULL,           -- 关联 recipes.recipe_id
    viewed_at   DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_view_history_user ON view_history(openid);
CREATE INDEX IF NOT EXISTS idx_view_history_recipe_id ON view_history(recipe_id);
-- 联合索引：优化按用户查询并按时间排序（SQLite 不支持索引中的 DESC，查询时使用 ORDER BY DESC）
CREATE INDEX IF NOT EXISTS idx_view_history_user_viewed ON view_history(openid, viewed_at);
-- 联合索引：优化按用户和菜谱查询
CREATE INDEX IF NOT EXISTS idx_view_history_user_recipe ON view_history(openid, recipe_id);
