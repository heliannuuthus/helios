...
import (
	...
	"github.com/heliannuuthus/helios/aegis/internal/dto"
	...
	"github.com/heliannuuthus/helios/aegis/iris"
...
type Handler struct {
	...
	mfaSvc          *iris.MFAService
	profileHandler  *iris.ProfileHandler
	pool            *async.Pool
}
...
// MFASvc 返回 MFA 服务
func (h *Handler) MFASvc() *iris.MFAService {
	return h.mfaSvc
}

// Profile 返回用户信息处理器
func (h *Handler) Profile() *iris.ProfileHandler {
...
func (h *Handler) GetContext(c *gin.Context) {
	...
	// 构建响应
	resp := &dto.AuthContextResponse{}

	if flow.Application != nil {
		resp.Application = &dto.ApplicationInfo{
...
func (h *Handler) Login(c *gin.Context) {
	...
	if err := h.resolveUser(ctx, flow); err != nil {
		if errors.Is(err, consts.ErrIdentifiedUser) {
...
func (h *Handler) GetIdentifyContext(c *gin.Context) {
	...
	resp := &dto.IdentifyResponse{
		Connection: flow.Connection,
		User: &dto.IdentifiedUser{
...
func (h *Handler) ConfirmIdentify(c *gin.Context) {
	var req dto.ConfirmIdentifyRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.errorResponse(c, autherrors.NewInvalidRequest(err.Error()))
		return
	}
...
func (h *Handler) Check(c *gin.Context) {
	ctStr := c.GetHeader(consts.HeaderAuthorization)
	if ctStr == "" {
		c.JSON(http.StatusUnauthorized, dto.CheckResponse{
			Error:   "unauthorized",
			Message: "missing CT",
		})
		return
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, dto.CheckResponse{
	...
	c.JSON(http.StatusOK, dto.CheckResponse{
		Results: results,
	})
}
...
func (h *Handler) LogoutGET(c *gin.Context) {
	clientID := c.Query(consts.QueryClientID)
	if clientID == "" {
...
func (h *Handler) authenticate(c *gin.Context, ctx context.Context, flow *types.AuthFlow, req *dto.LoginRequest) (bool, error) {
...
// 返回 consts.ErrIdentifiedUser 表示识别到已有用户，需前端确认关联
func (h *Handler) resolveUser(ctx context.Context, flow *types.AuthFlow) error {
	...
	if len(allIdentities) == 0 {
		...
		if existingUser != nil {
			...
			return consts.ErrIdentifiedUser
		}
...
func getAuthSessionCookie(c *gin.Context) (string, error) {
	return c.Cookie(consts.AuthSessionCookie)
}
