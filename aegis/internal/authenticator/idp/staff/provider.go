// Package staff provides the B-end staff password login IDP.
package staff
...
func (p *Provider) Login(ctx context.Context, proof string, params ...any) (*models.TUserInfo, error) {
	...
	logger.Infof("[staff] 登录请求 - Identifier: %s", maskIdentifier(identifier))
...
func (p *Provider) loginByPassword(ctx context.Context, identifier, password string) (*models.TUserInfo, error) {
	...
	if err != nil {
		logger.Warnf("[staff] 用户不存在 - Identifier: %s, Error: %v", maskIdentifier(identifier), err)
		return nil, errors.New("invalid credentials")
	}

	if cred.Status != 0 {
		logger.Warnf("[staff] 用户已禁用 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("user is disabled")
	}

	if cred.PasswordHash == "" {
		logger.Warnf("[staff] 用户未设置密码 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("password not set, please use other login methods")
	}

	if err := bcrypt.CompareHashAndPassword([]byte(cred.PasswordHash), []byte(password)); err != nil {
		logger.Warnf("[staff] 密码错误 - Identifier: %s", maskIdentifier(identifier))
		return nil, errors.New("invalid credentials")
	}

	logger.Infof("[staff] 登录成功 - Identifier: %s, OpenID: %s", maskIdentifier(identifier), cred.OpenID)
	...
	return &models.TUserInfo{
		...
		Email:    cred.Email,
		Picture:  cred.Picture,
		RawData:  fmt.Sprintf(`{"identifier":"%s","type":"staff"}`, identifier),
	}, nil
}

// getCredential 获取 B 端平台人员凭证
func (p *Provider) getCredential(ctx context.Context, identifier string) (*credential, error) {
	user, identity, err := idp.ResolveUserIdentity(ctx, p.userSvc, p.identitySvc, idp.TypeStaff, identifier)
	if err != nil {
		return nil, err
	}
	return &credential{
