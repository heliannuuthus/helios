...
// ==================== Iris 配置（原 iris/config） ====================

// IrisCfg 返回 Iris 配置单例
func IrisCfg() *baseconfig.Cfg {
	return baseconfig.Iris()
}

// GetIrisAegisAudience 获取 Iris 服务 audience（用于 token 验证）
func GetIrisAegisAudience() string {
	audience := IrisCfg().GetString("aegis.audience")
	if audience == "" {
		return "iris"
	}
	return audience
...
func GetIrisAegisSecretKey() string {
	return IrisCfg().GetString("aegis.secret-key")
}

// GetIrisAegisSecretKeyBytes 获取 Iris 服务密钥（48 字节 seed: 16-byte salt + 32-byte key）
